function step7richmondDist() {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = spreadsheet.getSheetByName("Unshipped2");
  var partialOrdersSheet = spreadsheet.getSheetByName("Partial orders2");
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    // Check if the current item has total quantity less than Richmond stock
    if (totalQuantity >= richmondStock) {
      orderStatus[orderNumber].allPartial = false;
    }
    
    // Check if the current item has total quantity equal to Richmond stock
    if (totalQuantity === richmondStock) {
